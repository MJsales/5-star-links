...
// real schedule (same live API the site uses), picks the matchup with the
// biggest gap between team rankings as the "best" pick, asks the same
// Gemini-backed endpoint the paid AI Sports Picks page uses for a real
// prediction, and posts it to Discord.
const fs = require('fs');
...
for (const g of games) {
  ...
  if (!best || gap > best.gap) best = { game: g, home, away, gap };
}
...
try {
  const aiRes = curlJSON(`${SITE}/api/ai-picks`, {
    method: 'POST',
    body: {
      message: `What is your best, most confident pick for the next game on the slate: ${matchup}?`,
  ...
  console.log('ai-picks call failed:', e.message);
}

if (!reply) {
  console.log('No AI reply, skipping post.');
  process.exit(0);
}
